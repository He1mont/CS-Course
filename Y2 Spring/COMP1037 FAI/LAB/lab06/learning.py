...
def cross_validation(learner, dataset, size=None, k=10, trials=1):
    ...
    if trials > 1:
        ...
        for t in range(trials):
            errT, errV = cross_validation(learner, dataset, size, k, trials=1)
            trial_errT += errT
            trial_errV += errV
...
def LinearLearner(dataset, learning_rate=0.01, epochs=100):
   ...
   # X_col = [dataset.values[i] for i in idx_i]  # vertical columns of X
    X_tran = list(zip(*examples)) # vertical columns of X updated by qz
    X_col = [X_tran[i] for i in idx_i]
    ...
    # updated by qz
    X_row = list(zip(*X_col))
    
    for epoch in range(epochs):
        err = []
        # pass over all examples updated by qz,
